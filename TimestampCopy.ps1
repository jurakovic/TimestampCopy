...
'@
    ...
    $inMs = New-Object IO.MemoryStream (,[byte[]]$compressedBytes)
    $outMs = New-Object IO.MemoryStream
    $gzip = New-Object IO.Compression.GZipStream $inMs, ([IO.Compression.CompressionMode]::Decompress)
    $gzip.CopyTo($outMs)
    $gzip.Close()
    $iconBytes = $outMs.ToArray()
